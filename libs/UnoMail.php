...
class UnoMail
{
  ...
  /**
   * レスポンスヘッダー生成
   * 私は1つの処理に対して1メソッドと考えています
   * アクセス修飾子は上から public private protected の順に記載します
   *
  ...
  public function checkInputsData()
  {
    ...
    echo json_encode( '情報の入力が確認されました', JSON_UNESCAPED_UNICODE );
    return true;
  }
  ...
  {
    // 送信チェック result　と　reply_result がOKなら complete!
    if( ( $result && $replyResult ) ) {
      header( self::REQUEST_HEADER );
      return json_encode( 'complete!', JSON_UNESCAPED_UNICODE );
    }

    // 私はなるべくネストが深くならない書き方を意識しています。
    // https://qiita.com/DeployCat/items/1ec901864d4ab11c8d6f
    // IF の中にIF が入ってしまう時は、主にこのような状況です。
    /** 例1
    * if (条件1) {
    *   if (条件2) {
    *    trueの処理
    ...
    * }
    */
    // 条件1,条件2を false の内容にしてreturnすると、ネストが深くならずに済みます。
    // 処理が下に行けば行くほど、条件を満たしているという意味にもなります。
    /** 例2
    * if (!条件1) {
    *   return;
    * }
  ...
  {
    header( self::REQUEST_HEADER );
    return json_encode( '不正なアクセスです!', JSON_UNESCAPED_UNICODE );
  }
  ...
  *********************************** */
  ...
  // 以降の処理には何も手を加えていません。
  // php docsも記述していませんので、必要に応じてご自身で修正してみてください！
